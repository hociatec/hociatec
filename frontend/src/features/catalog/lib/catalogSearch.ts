...
export const emptyCatalogMeta: CatalogSearchMeta = { page: 1, perPage: CATALOG_PAGE_SIZE, total: 0, totalPages: 1 };
...
export const formatCatalogResultsSummary = (total: number, query: string, noun: string) => {
  ...
  return query.trim() ? `${total} ${noun}${suffix} pour « ${query.trim()} »` : `${total} ${noun}${suffix} disponible${suffix}`;
};
