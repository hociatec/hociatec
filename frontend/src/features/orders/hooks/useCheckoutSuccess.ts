    ...
    const poll = async () => {
      try {
        ...
        if (result.status === 'failed' || result.status === 'expired') {
          setStatus('error');
          setMessage(
            'Le paiement n’a pas pu être confirmé. Vous pouvez réessayer depuis le panier.',
          );
          return;
        }
        attempts += 1;
        ...
        if (attempts < 20) window.setTimeout(() => void poll(), 2000);
          ...
          setMessage(
            'Le paiement est en cours de finalisation. Rechargez cette page dans quelques secondes.',
          );
      } catch (error) {
        if (!cancelled) {
          setStatus('error');
          setMessage(
            error instanceof Error ? error.message : 'Impossible de vérifier le paiement.',
          );
        }
      }
    };
