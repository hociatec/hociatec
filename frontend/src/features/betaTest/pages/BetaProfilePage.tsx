...
export const BetaProfilePage = () => {
  ...
  useEffect(() => { void fetchMyBetaProfile().then(profile => setForm({ motivation: String(profile.motivation ?? ''), testingExperience: String(profile.testingExperience ?? ''), bugDescriptionAbility: String(profile.bugDescriptionAbility ?? ''), technicalKnowledge: String(profile.technicalKnowledge ?? ''), availability: Array.isArray(profile.availability) ? profile.availability as string[] : ['flexible'], accessibilityNeed: String(profile.accessibilityNeed ?? 'none'), assistiveTools: Array.isArray(profile.assistiveTools) ? profile.assistiveTools as string[] : [], devices: Array.isArray(profile.devices) ? profile.devices as string[] : ['windows'], browsers: Array.isArray(profile.browsers) ? profile.browsers as string[] : ['chrome'], testingTypes: Array.isArray(profile.testingTypes) ? profile.testingTypes as string[] : ['bugs'], betaConsent: true })).catch(reason => setError(reason instanceof Error ? reason.message : 'Impossible de charger le profil.')); }, []);
