...
export const selectQuoteToAnswer = (data: DashboardData) =>
  data.quotes.find((quote) => (quote.statusCode ?? quote.status) === 'sent' && !quote.convertedOrder) ?? null;

export const selectNextAppointment = (data: DashboardData) => data.appointments[0] ?? null;

export const selectNextTraining = (data: DashboardData, loadedAtMs: number) => data.trainings
...
export const selectDashboardActions = (data: DashboardData, loadedAtMs: number): DashboardAction[] => {
  ...
  if (nextAppointment) {
    actions.push({ kind: 'appointment', title: 'Préparer votre rendez-vous', detail: formatOptionalFrenchDateTime(nextAppointment.startAt), to: '/appointments/me' });
  }
  if (quoteToAnswer) {
    actions.push({ kind: 'quote', title: `Répondre au devis ${quoteToAnswer.number ?? `#${quoteToAnswer.id}`}`, detail: 'Accepter ou refuser la proposition', to: `/quotes/me/${quoteToAnswer.id}` });
  }
  if (nextTraining) {
    actions.push({ kind: 'training', title: 'Voir votre formation à venir', detail: `${nextTraining.session.training.title} · ${formatOptionalFrenchDateTime(nextTraining.scheduledStartsAt)}`, to: `/trainings/me/${nextTraining.id}` });
  }
  return actions;
};

export const normalizeConversionPoints = (value: string) => Math.floor((Number.parseInt(value, 10) || 0) / 100) * 100;
