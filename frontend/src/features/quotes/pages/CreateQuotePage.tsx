            ...
            <section>
              ...
              {productLoading && (
                <p className="text-sm text-stone-500">
                  Recherche des produits et services correspondants...
                </p>
              )}
                          ...
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <div className="text-xs text-stone-500">
                                {formatEuroCents(s.priceCents ?? 0)}
                              </div>
                            </div>
                            <button
                              type="button"
                              className={
                                (form.items ?? []).some(
                                  (it) => it.type === 'service' && it.serviceId === s.id,
                                )
                        ...
                        <div key={p.id} className="rounded border border-brand-100 p-2">
                          <div className="flex items-center justify-between gap-3">
                            <div>
                              ...
                              <div className="text-xs text-stone-500">
                                {formatEuroCents(p.effectivePriceCents ?? p.priceCents ?? 0)}
                                {p.sellingType === 'rental' ? ' / mois' : ''}
                              </div>
                            </div>
                            {p.sellingType === 'rental' ? (
                              ...
                              <div className="inline-flex items-center gap-2">
                                ...
                                <input
                                  ...
                                  value={Math.max(
                                    1,
                                    form.items[findProductItemIndex(p.id)]?.quantity ?? 1,
                                  )}
                                  onChange={(e) => {
                                    const index = findProductItemIndex(p.id);
                                    if (index < 0) return;
