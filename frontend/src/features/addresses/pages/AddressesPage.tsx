  ...
  const renderB2bFields = (
    ...
    <>
      <div className="rounded-2xl border border-brand-100 bg-brand-50 p-4">
        <p className="text-sm font-semibold text-brand-900">Informations de facturation professionnelle</p>
      ...
      <label className="grid gap-2 text-sm font-medium text-stone-700">
        Société
        <input
          className="rounded-xl border border-brand-100 px-4 py-3 outline-none transition focus:border-brand-300"
      ...
      <div className="mx-auto max-w-6xl px-4 py-10">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.05fr_1.4fr]">
          <section className="rounded-xl border border-brand-100 bg-white p-6 shadow-sm">
            <div className="mb-5">
              <h2 className="text-xl font-semibold text-brand-900">Ajouter une adresse</h2>
              <p className="mt-1 text-sm text-stone-600">
            ...
            <div className="grid gap-4">
              ...
              <button
                type="button"
                className="inline-flex items-center justify-center rounded-full bg-brand-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-brand-800 disabled:cursor-not-allowed disabled:opacity-60"
                    ...
                    <li key={it.id} className="rounded-2xl border border-brand-100 p-4">
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          <h3 className="text-base font-semibold text-brand-900">{it.name}</h3>
                          {it.isDefault && (
                        ...
                        {it.company ? <p className="text-sm font-medium text-stone-800">{it.company}</p> : null}
                        <p className="text-sm text-stone-700">{it.address}</p>
                        <p className="text-sm text-stone-600">
                          {it.postalCode} {it.city}
                        </p>
                        {it.companySiren ? <p className="text-xs text-stone-500">SIREN : {it.companySiren}</p> : null}
                        {it.companyVatNumber ? <p className="text-xs text-stone-500">TVA : {it.companyVatNumber}</p> : null}
                        {it.purchaseOrderNumber ? (
                          <p className="text-xs text-stone-500">Bon de commande : {it.purchaseOrderNumber}</p>
                        ) : null}
                      </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-brand-900/50 px-4 py-6">
            <div className="w-full max-w-xl rounded-xl bg-white p-6 shadow-2xl">
              ...
              <div className="mt-6 flex flex-wrap justify-end gap-3">
                <button
                  type="button"
                  className="inline-flex items-center justify-center rounded-full border border-brand-200 px-5 py-3 text-sm font-semibold text-stone-700 transition hover:border-brand-600"
