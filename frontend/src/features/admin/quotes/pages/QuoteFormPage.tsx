...
const DEFAULT_QUOTE_CONDITIONS = `Le prÃ©sent devis constitue une offre valable jusqu'Ã  la date de fin de validitÃ© qui y figure. Il devient contractuel Ã  compter de son acceptation expresse par le client.
Le devis est Ã©tabli sur la base des informations communiquÃ©es par le client. Toute prestation, fourniture ou demande complÃ©mentaire non prÃ©vue au devis initial fera l'objet d'un accord Ã©crit complÃ©mentaire ou d'un avenant.
Sauf stipulation particuliÃ¨re, les dÃ©lais d'exÃ©cution ou de livraison sont indicatifs et courent Ã  compter de la rÃ©ception de l'acceptation du devis et, le cas Ã©chÃ©ant, de l'acompte prÃ©vu.
Sauf mention contraire, les prix sont exprimÃ©s en euros. Les taxes applicables sont celles en vigueur au jour de la facturation.
Pour les clients professionnels uniquement, tout retard de paiement pourra entraÃ®ner l'application de pÃ©nalitÃ©s de retard exigibles sans rappel, calculÃ©es au taux de refinancement de la BCE majorÃ© de 10 points, ainsi qu'une indemnitÃ© forfaitaire de 40 euros pour frais de recouvrement.
Pour les clients consommateurs, les garanties lÃ©gales applicables demeurent celles prÃ©vues par la loi.`;
...
export const QuoteFormPage = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const msg = e?.message ?? 'Ã‰chec de sauvegarde.';
      setError(msg);
