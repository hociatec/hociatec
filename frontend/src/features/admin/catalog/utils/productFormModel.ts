...
type ProductPayloadResult = { payload: UpsertProductPayload; error?: never } | { payload?: never; error: string };
...
export const buildProductPayload = ({
  ...
  if (Number.isNaN(stockValue) || stockValue < 0) return { error: 'Le stock doit être un entier positif.' };
  ...
  if (selectedBrand === null) return { error: 'La marque est obligatoire. Recherchez puis cochez une marque existante.' };
  ...
  if (existingVariantKeys.has(currentVariantKey)) {
    return { error: `La variante ${formatVariantConflictLabel(form.color, normalizedStorage)} existe déjà.` };
  }
  ...
  for (const row of variantPayload) {
    ...
    if (existingVariantKeys.has(key) || incomingVariantKeys.has(key)) {
      return { error: `La variante ${formatVariantConflictLabel(row.color, row.storageCapacity)} existe déjà.` };
    }
