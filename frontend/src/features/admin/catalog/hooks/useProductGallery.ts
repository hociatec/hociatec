...
export const useProductGallery = () => {
  ...
  const [galleryPreviews, setGalleryPreviews] = useState<Array<string | null>>(emptyGallery<string>);
  ...
  const onFileChange = (index: number, fileList: FileList | null) => {
    ...
    setGalleryFiles((previous) => previous.map((value, itemIndex) => itemIndex === index ? file : value));
    if (file) {
      const objectUrl = URL.createObjectURL(file);
      objectUrlsRef.current[index] = objectUrl;
      setGalleryPreviews((previous) => previous.map((value, itemIndex) => itemIndex === index ? objectUrl : value));
      setGalleryToRemove((previous) => previous.filter((value) => value !== index));
      return;
    }

    const fallback = initialGallery[index];
    setGalleryPreviews((previous) => previous.map((value, itemIndex) => itemIndex === index ? fallback : value));
  ...
  const remove = (index: number) => {
    if (objectUrlsRef.current[index]) URL.revokeObjectURL(objectUrlsRef.current[index]!);
    objectUrlsRef.current[index] = null;
    setGalleryFiles((previous) => previous.map((value, itemIndex) => itemIndex === index ? null : value));
    setGalleryPreviews((previous) => previous.map((value, itemIndex) => itemIndex === index ? null : value));
    setGalleryToRemove((previous) => initialGallery[index]
      ? Array.from(new Set([...previous, index]))
      : previous.filter((value) => value !== index));
  };

  return { galleryFiles, galleryPreviews, initialGallery, galleryToRemove, hydrate, reset, onFileChange, remove };
};
