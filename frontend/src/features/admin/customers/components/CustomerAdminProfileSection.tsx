    ...
    <div className="border-b border-brand-100 bg-brand-50/80 px-5 py-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex items-center gap-3">
          {saveMessage ? (
            <div
              className={`rounded-full px-3 py-1 text-xs font-medium ${saveState === 'error' ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'}`}
            >
              {saveMessage}
            </div>
          ) : null}
      ...
      <div className="mb-5 flex items-center justify-between gap-3">
        <div>
          ...
          <div className="text-xs text-stone-500">
            Mets à jour les tags et les notes, puis enregistre.
          </div>
        </div>
        <div className="flex items-center gap-3">
          {saveState === 'saving' ? (
            <div className="text-xs text-stone-500">Enregistrement en cours...</div>
          ) : null}
          <button
            type="button"
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.15fr)_minmax(300px,0.85fr)]">
        <div className="rounded-2xl border border-brand-100 bg-white p-4">
          ...
          <input
            ...
            onChange={(event) => onAdminTagsInputChange(event.target.value)}
            ...
            <div className="flex min-h-14 flex-wrap gap-2">
              {parsedTags.length > 0 ? (
                parsedTags.map((tag) => (
                  <span
                    key={tag}
                    className="rounded-full border border-brand-100 bg-white px-3 py-1.5 text-xs font-medium text-stone-700 shadow-sm"
                  >
                    {tag}
                  </span>
                ))
