        ...
        <div className="space-y-6">
          <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            <div className="border-b border-slate-200 bg-slate-950 px-6 py-5 text-white">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-300">Bons de réduction</p>
            ...
            <div className="grid gap-6 px-6 py-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)]">
              <div className="space-y-4 rounded-3xl border border-slate-200 p-5">
                <div>
                  <div className="text-sm font-semibold text-slate-900">Configuration du bon</div>
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                <h2 className="text-lg font-semibold text-slate-900">Historique des bons envoyés</h2>
                <p className="mt-1 text-sm text-slate-500">Retrouve les offres déjà créées pour ce client, leur statut et leur envoi.</p>
              </div>
            ...
            {vouchers.length === 0 ? (
              <p className="text-sm text-slate-500">Aucun bon de réduction créé pour ce client.</p>
            ) : (
                ...
                {vouchers.map((voucher) => (
                  <div key={voucher.id} className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
                    <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                      <div>
                        <div className="font-semibold text-slate-900">{voucher.name}</div>
                        <div className="text-sm text-slate-600">
                          Code {voucher.code} · {voucher.discountType === 'percent' ? `${voucher.discountValue}%` : formatPrice(voucher.discountValue)}
                        </div>
                        <div className="text-sm text-slate-500">
                          Créé le {new Date(voucher.createdAt).toLocaleString('fr-FR')}
                          {voucher.sentAt ? ` · envoyé le ${new Date(voucher.sentAt).toLocaleString('fr-FR')}` : ' · non envoyé'}
                        </div>
                        {voucher.description ? <div className="mt-1 text-sm text-slate-600">{voucher.description}</div> : null}
                      </div>
                      <div className="flex flex-wrap gap-2 text-xs">
                        <span className={`rounded-full px-3 py-1 ${voucher.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-700'}`}>
                          {voucher.isActive ? 'Actif' : 'Inactif'}
                        </span>
                        {voucher.endsAt ? (
                          <span className="rounded-full bg-white px-3 py-1 text-slate-600">
                            Fin {new Date(voucher.endsAt).toLocaleDateString('fr-FR')}
                          </span>
                        ) : null}
                        <button
                          type="button"
