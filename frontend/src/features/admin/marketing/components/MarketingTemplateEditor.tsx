...
import type { MarketingSegmentDefinition, MarketingTemplatePayload } from '../api';

type MarketingTemplateEditorProps = {
  form: MarketingTemplatePayload;
  setForm: Dispatch<SetStateAction<MarketingTemplatePayload>>;
...
export const MarketingTemplateEditor = ({
  ...
  <form onSubmit={onSubmit} className="register-form-card form-card-grid">
    ...
    <label className="register-form__field">
      ...
      <select className="register-form__input" value={form.scenarioKey} onChange={(event) => setForm((prev) => ({ ...prev, scenarioKey: event.target.value }))}>
        <optgroup label="Emails transactionnels">
          {Object.entries(segments).filter(([, segment]) => segment.type === 'transactional').map(([key, segment]) => <option key={key} value={key}>{segment.label}</option>)}
        </optgroup>
