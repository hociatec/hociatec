          ...
          <form onSubmit={campaign.handleSend} className="register-form-card form-card-grid">
            <label className="register-form__field">
              ...
              <input
                ...
                onChange={(event) =>
                  campaign.setForm((prev) => ({ ...prev, name: event.target.value }))
                }
              />
            </label>
            ...
            {campaign.form.segmentKey === 'customers_with_pending_reviews' && (
              <label className="register-form__field">
                ...
                <input
                  ...
                  onChange={(event) =>
                    campaign.setForm((prev) => ({
                      ...prev,
                      minimumPendingReviews: event.target.value,
                    }))
                  }
                />
              </label>
            ...
            <label className="register-form__field">
              ...
              <input
                ...
                onChange={(event) =>
                  campaign.setForm((prev) => ({ ...prev, subject: event.target.value }))
                }
              />
            </label>
            ...
            <label className="register-form__field">
              ...
              <textarea
                ...
                onChange={(event) =>
                  campaign.setForm((prev) => ({ ...prev, htmlBody: event.target.value }))
                }
              />
            </label>
            ...
            <label className="register-form__field">
              ...
              <textarea
                ...
                onChange={(event) =>
                  campaign.setForm((prev) => ({ ...prev, textBody: event.target.value }))
                }
              />
            </label>
          ...
          <div className="space-y-6">
              ...
              {campaign.preview ? (
                <>
                  <div className="rounded-2xl bg-brand-50 px-4 py-3 text-sm text-stone-700">
                    <strong>{campaign.preview.count}</strong> destinataire(s).{' '}
                  ...
                  <div className="space-y-2">
                    {campaign.preview.recipients.map((recipient) => (
                      <div
                        key={recipient.id}
                        className="rounded-xl border border-brand-100 px-3 py-2 text-sm"
                      >
                ...
                <p className="text-sm text-stone-500">
                  Lancez une prévisualisation pour voir le volume et quelques exemples.
                </p>
              )}
            </div>
            ...
            <div className="register-form-card form-card-grid">
              <div className="flex items-center justify-between gap-3">
                ...
                <Link
                  to="/admin/marketing/templates"
                  className="text-sm font-semibold text-stone-700 hover:text-brand-900"
                >
              ...
              {campaign.templatesForSegment.length === 0 ? (
                ...
                <div className="space-y-3">
                  {campaign.templatesForSegment.slice(0, 4).map((template) => (
                    <div
                      key={template.id}
                      className="rounded-2xl border border-brand-100 px-4 py-4"
                    >
                      ...
                      <div className="mt-3 flex flex-wrap gap-3 text-sm">
                        <Link
                          ...
                          className="font-semibold text-stone-700 hover:text-brand-900"
                        >
                          Voir le détail
                        </Link>
                        <Link
