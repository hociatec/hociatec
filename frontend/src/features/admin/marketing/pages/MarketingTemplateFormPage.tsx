        ...
        <form onSubmit={handleSubmit} className="register-form-card form-card-grid">
          ...
          <label className="register-form__field">
            ...
            <select
              ...
              onChange={(event) =>
                setForm((prev) => ({ ...prev, scenarioKey: event.target.value }))
              }
            >
          ...
          <label className="register-form__field">
            ...
            <input
              ...
              onChange={(event) =>
                setForm((prev) => ({ ...prev, subjectTemplate: event.target.value }))
              }
            />
          </label>
          ...
          <label className="register-form__field">
            ...
            <textarea
              ...
              rows={10}
              value={form.htmlBody}
              onChange={(event) => setForm((prev) => ({ ...prev, htmlBody: event.target.value }))}
            />
          </label>
          ...
          <p className="text-sm text-stone-500">
            Variables disponibles selon scénario: {'{{first_name}}'}, {'{{last_name}}'},{' '}
            {'{{full_name}}'}, {'{{email}}'}, {'{{activation_url}}'}, {'{{activation_expires_in}}'},{' '}
            {'{{password_reset_url}}'}, {'{{password_reset_expires_in}}'}, {'{{order_number}}'},{' '}
            {'{{order_status}}'}, {'{{order_status_label}}'}, {'{{order_email_status_title}}'},{' '}
            {'{{order_payment_instruction}}'}, {'{{order_payment_next_step}}'}, {'{{quote_number}}'}
            , {'{{quote_total_eur}}'}, {'{{quote_valid_until}}'}, {'{{quote_detail_url}}'},{' '}
            {'{{customer_name}}'}, {'{{order_origin_sentence}}'}, {'{{previous_order_status}}'},{' '}
            {'{{previous_order_status_label}}'}, {'{{invoice_number}}'}, {'{{invoice_date}}'},{' '}
            {'{{order_total_eur}}'}, {'{{order_created_at}}'}, {'{{billing_name}}'},{' '}
            {'{{purchase_order_number}}'}, {'{{order_detail_url}}'}, {'{{orders_list_url}}'},{' '}
            {'{{app_frontend_url}}'}, {'{{order_count}}'}, {'{{total_spent_eur}}'},{' '}
            {'{{last_order_number}}'}, {'{{last_order_date}}'}, {'{{days_since_last_order}}'},{' '}
            {'{{pending_reviews_count}}'}, {'{{voucher_name}}'}, {'{{voucher_code}}'},{' '}
            {'{{voucher_description}}'}, {'{{voucher_discount_type}}'},{' '}
            {'{{voucher_discount_value}}'}, {'{{voucher_value_label}}'}, {'{{voucher_starts_at}}'},{' '}
            {'{{voucher_ends_at}}'}, {'{{voucher_is_active}}'}, {'{{shop_url}}'}, {'{{cart_url}}'},{' '}
            {'{{product_name}}'}, {'{{product_summary}}'}, {'{{product_price_eur}}'},{' '}
            {'{{product_url}}'}, {'{{contact_name}}'}, {'{{contact_email}}'},{' '}
            {'{{contact_subject}}'}, {'{{contact_message}}'}, {'{{mailer_from}}'}.
          </p>
