...
const emptySupportForm: SupportForm = {
  customerId: '', orderId: '', subject: '', reason: 'other', message: '', internalNotes: '',
};
...
export const useAdminOperations = () => {
  ...
  const refresh = useCallback(async () => {
    ...
    setMessage(null);
    try {
      const [overviewData, supportData, refundData, stockData, emailData, fulfillmentData] = await Promise.all([
        fetchOperationsOverview(), fetchSupportRequests(), fetchRefunds(), fetchStockMovements(),
        fetchEmailLogs(), fetchFulfillmentOrders(),
      ]);
  ...
  const submitSupport = useCallback(() => void runAction(
    async () => {
  ...
  const submitRefund = useCallback(() => void runAction(
    async () => {
      await createRefund({
        orderId: Number(refundForm.orderId), amountCents: Number(refundForm.amountCents),
        reason: refundForm.reason, internalNotes: refundForm.internalNotes,
      });
  ...
  const submitStock = useCallback(() => void runAction(
    async () => {
      await createStockMovement({
        productId: Number(stockForm.productId), delta: Number(stockForm.delta),
        reason: stockForm.reason, note: stockForm.note,
      });
  ...
  const submitBulk = useCallback(() => {
    ...
    void runAction(
      async () => { await bulkUpdateOrderStatus(ids, bulkForm.status); setBulkForm(emptyBulkForm); },
      `${ids.length} commande(s) mise(s) à jour.`, 'Erreur action groupée.',
    );
  }, [bulkForm, runAction]);
  ...
  const submitQuoteConversion = useCallback(() => {
    ...
    void convertQuoteToOrder(reference)
      .then((order) => { setMessage(`Devis converti en commande ${order.number}.`); setQuoteReference(''); navigate(`/admin/orders/${order.id}`); })
      .catch((error) => { setQuoteConversionStatus('error'); setQuoteConversionMessage(getHttpErrorMessage(error, 'Erreur conversion devis.')); })
      .finally(() => setQuoteConversionStatus('idle'));
  }, [navigate, quoteReference]);

  const submitShipOrder = useCallback((orderId: number) => {
    const payload = shippingForms[orderId] ?? emptyShippingForm;
    void runAction(
      async () => { await shipFulfillmentOrder(orderId, payload); setShippingForms((current) => ({ ...current, [orderId]: emptyShippingForm })); },
      'Commande marquée comme expédiée.', 'Erreur expédition.',
    );
  }, [runAction, shippingForms]);

  const submitSupportReply = useCallback((supportId: number) => {
    const payload = supportReplies[supportId] ?? { subject: `Réponse SAV #${supportId}`, message: '' };
    void runAction(
      async () => { await replySupportRequest(supportId, { ...payload, status: 'waiting_customer' }); setSupportReplies((current) => ({ ...current, [supportId]: { subject: '', message: '' } })); },
      'Réponse SAV envoyée au client.', 'Erreur réponse SAV.',
    );
  }, [runAction, supportReplies]);

  const submitStripeRefund = useCallback((refundId: number) => {
    void runAction(
      async () => {
        const refund = await processStripeRefund(refundId, { confirmation: refundConfirmations[refundId] ?? '' });
      ...
      'Remboursement Stripe traité.', 'Erreur remboursement Stripe.',
    );
  }, [refundConfirmations, runAction]);

  const submitStockThreshold = useCallback((productId: number) => {
    void runAction(
      () => updateLowStockThreshold(productId, Number(stockThresholds[productId])).then(() => undefined),
      'Seuil de stock faible mis à jour.', 'Erreur seuil stock.',
    );
  }, [runAction, stockThresholds]);

  const updateRefundStatus = useCallback((refundId: number, nextStatus: string) => {
    void runAction(() => updateRefund(refundId, { status: nextStatus }).then(() => undefined), 'Statut remboursement mis à jour.', 'Erreur statut remboursement.');
  }, [runAction]);

  const updateSupportStatus = useCallback((supportId: number, nextStatus: string) => {
    void runAction(() => updateSupportRequest(supportId, { status: nextStatus }).then(() => undefined), 'Statut SAV mis à jour.', 'Erreur statut SAV.');
  }, [runAction]);

  const failedEmails = useMemo(() => emails.filter((email) => email.status === 'failed').length, [emails]);
  const hasPriorities = Boolean(
    (overview?.support.openCount ?? 0) > 0 || (overview?.refunds.pendingCount ?? 0) > 0 ||
    (overview?.stock.lowStockCount ?? 0) > 0 || failedEmails > 0,
  );

  return {
    overview, support, refunds, stock, emails, fulfillmentOrders, status, message, refresh,
    failedEmails, hasPriorities, supportForm, refundForm, stockForm, bulkForm, quoteReference,
    quoteConversionStatus, quoteConversionMessage, shippingForms, supportReplies, refundConfirmations,
    stockThresholds, setSupportForm, setRefundForm, setStockForm, setBulkForm, setQuoteReference,
    setShippingForms, setSupportReplies, setRefundConfirmations, setStockThresholds,
    submitSupport, submitRefund, submitStock, submitBulk, submitQuoteConversion, submitShipOrder,
    submitSupportReply, submitStripeRefund, submitStockThreshold, updateRefundStatus, updateSupportStatus,
  };
};
