...
export const OperationsSupportCard = ({ supportForm, setSupportForm, submitSupport }: {
  ...
  <ActionCard title="Créer un dossier SAV" description="À utiliser quand un client signale un problème ou quand une commande nécessite un suivi manuel.">
    <div className="grid gap-3 sm:grid-cols-2">
      <Field label="ID client" helper="Obligatoire. À récupérer depuis la fiche client."><input className={inputClass} inputMode="numeric" placeholder="Ex. 42" value={supportForm.customerId} onChange={(event) => setSupportForm((current) => ({ ...current, customerId: event.target.value }))} /></Field>
      <Field label="ID commande" helper="Optionnel si la demande n’est pas liée à une commande."><input className={inputClass} inputMode="numeric" placeholder="Ex. 128" value={supportForm.orderId} onChange={(event) => setSupportForm((current) => ({ ...current, orderId: event.target.value }))} /></Field>
