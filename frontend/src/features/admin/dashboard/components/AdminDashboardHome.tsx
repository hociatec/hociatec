  ...
  <section className="space-y-4">
    <div className="flex flex-wrap items-end justify-between gap-3">
      <div>
        ...
        <p className="text-sm text-stone-400">
          Devis acceptés, commandes à régler, emails et paiements récents.
        </p>
      </div>
    ...
    <div className="grid gap-3 xl:grid-cols-2">
      {(dashboard.notifications ?? []).length === 0 ? (
        <div className="rounded-2xl border border-brand-700 bg-brand-800/50 p-5 text-sm text-stone-500">
          ...
          return (
            ...
            >
              <div className="mt-0.5">
                {isDanger || isAction ? (
                  <CircleAlert
                    className={`h-5 w-5 ${isDanger ? 'text-red-300' : 'text-amber-300'}`}
                  />
                ) : (
        ...
        payments.map((payment) => (
          ...
          >
            ...
            <div className="flex flex-col items-start gap-2 md:items-end">
              ...
              <div className="text-xs text-stone-400">
                {formatFrenchDateTime(payment.createdAt)}
              </div>
