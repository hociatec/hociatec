...
import { type AdminCustomerAddressDto, type AdminCustomerDetailDto } from '@/features/admin/customers/api';
...
export const CustomerQuickActions = ({
  ...
  <section className="rounded-2xl border border-brand-100 p-4">
    <div className="flex flex-wrap gap-3">
      <button type="button" className="inline-flex items-center rounded-full border border-brand-200 px-4 py-2 text-sm font-semibold text-stone-700 transition hover:border-brand-600" onClick={onToggleEmail}>
        {emailOpen ? 'Fermer l’e-mail' : 'Envoyer un e-mail'}
      </button>
      {customer.phoneNumber ? (
        <a className="inline-flex items-center rounded-full border border-brand-200 px-4 py-2 text-sm font-semibold text-stone-700 transition hover:border-brand-600" href={`tel:${normalizePhoneLink(customer.phoneNumber)}`}>
          Appeler le client
        </a>
      ) : null}
      {latestOrder ? (
        <Link className="inline-flex items-center rounded-full border border-brand-200 px-4 py-2 text-sm font-semibold text-stone-700 transition hover:border-brand-600" to={`/admin/orders/${latestOrder.id}`}>
          Ouvrir la dernière commande
        </Link>
      ) : null}
      {customer.ordersCount > 0 ? (
        <Link className="inline-flex items-center rounded-full border border-brand-200 px-4 py-2 text-sm font-semibold text-stone-700 transition hover:border-brand-600" to={`/admin/orders?search=${encodeURIComponent(customer.email)}`}>
          Rechercher ses commandes
        </Link>
      ) : null}
      <Link className="inline-flex items-center rounded-full border border-brand-200 px-4 py-2 text-sm font-semibold text-stone-700 transition hover:border-brand-600" to={`/admin/customers/${customer.id}/vouchers/new`}>
  ...
  <section className="overflow-hidden rounded-xl border border-brand-100 bg-white shadow-sm">
    <div className="border-b border-brand-100 bg-brand-900 px-6 py-5 text-white">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-stone-500">Messagerie client</p>
          <h2 className="mt-1 text-xl font-semibold">Envoyer un e-mail manuel</h2>
          <p className="mt-2 text-sm text-stone-500">Message direct vers {customer.fullName} ({customer.email}).</p>
        </div>
    ...
    <div className="grid gap-6 px-6 py-6 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
      <div className="rounded-xl border border-brand-100 bg-brand-50 p-5">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-stone-500">Préremplis les cas les plus fréquents sans repartir de zéro.</p>
          </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {customerEmailPresets.map((preset) => (
            <button key={preset.id} type="button" className="rounded-full border border-brand-200 bg-white px-3 py-1.5 text-xs font-medium text-stone-700 transition hover:border-brand-600 hover:bg-brand-50" onClick={() => onApplyPreset(preset)}>
              {preset.label}
            </button>
          ))}
        </div>
      ...
      <div className="space-y-4 rounded-xl border border-brand-100 p-5">
        <div>
          ...
          <p className="mt-1 text-sm text-stone-500">Le sujet et le contenu sont envoyés tels quels au client.</p>
        </div>
        <label className="register-form__field">
          <span className="register-form__label">Sujet</span>
          <input className="register-form__input" value={emailForm.subject} onChange={(event) => onEmailFormChange({ ...emailForm, subject: event.target.value })} placeholder="Sujet de l’email" />
        </label>
        <label className="register-form__field">
          <span className="register-form__label">Contenu</span>
          <textarea className="register-form__input" rows={10} value={emailForm.message} onChange={(event) => onEmailFormChange({ ...emailForm, message: event.target.value })} placeholder="Rédige ton message ici..." />
        </label>
        <div className="flex flex-wrap gap-3 pt-2">
          <button type="button" className="inline-flex items-center rounded-full bg-brand-900 px-4 py-2 text-sm font-semibold text-white transition hover:bg-brand-800 disabled:opacity-60" onClick={onSendEmail} disabled={emailSending || emailForm.subject.trim() === '' || emailForm.message.trim() === ''}>
            {emailSending ? 'Envoi...' : 'Envoyer'}
          </button>
          {!emailOnlyView ? (
            <button type="button" className="inline-flex items-center rounded-full border border-brand-200 px-4 py-2 text-sm font-semibold text-stone-700 transition hover:border-brand-600" onClick={onClose}>
              Fermer
            </button>
          ) : null}
        </div>
...
export const CustomerOrdersSection = ({
  ...
  const openOrdersCount = orders.filter((order) => order.status === 'pending' || order.status === 'confirmed').length;
    ...
    <section className="rounded-2xl border border-brand-100 p-4">
      <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap gap-2 text-sm">
          <button type="button" className={`rounded-full px-3 py-1.5 ${orderFilter === 'all' ? 'bg-brand-900 text-white' : 'bg-brand-50 text-stone-700'}`} onClick={() => onOrderFilterChange('all')}>
            Toutes ({orders.length})
          </button>
          <button type="button" className={`rounded-full px-3 py-1.5 ${orderFilter === 'open' ? 'bg-brand-900 text-white' : 'bg-brand-50 text-stone-700'}`} onClick={() => onOrderFilterChange('open')}>
            En cours ({openOrdersCount})
          </button>
          <button type="button" className={`rounded-full px-3 py-1.5 ${orderFilter === 'delivered' ? 'bg-brand-900 text-white' : 'bg-brand-50 text-stone-700'}`} onClick={() => onOrderFilterChange('delivered')}>
            Livrées ({deliveredOrdersCount})
          </button>
          <button type="button" className={`rounded-full px-3 py-1.5 ${orderFilter === 'cancelled' ? 'bg-brand-900 text-white' : 'bg-brand-50 text-stone-700'}`} onClick={() => onOrderFilterChange('cancelled')}>
      ...
      {orders.length === 0 ? (
        ...
        <div className="space-y-3">
          {filteredOrders.map((order) => (
            <div key={order.id} className="flex flex-col gap-3 rounded-2xl bg-brand-50 p-4 md:flex-row md:items-center md:justify-between">
              <div>
                <div className="font-semibold text-brand-900">{order.number}</div>
                <div className="text-sm text-stone-600">
                  {formatFrenchDateTime(order.createdAt)} · {order.statusLabel ?? formatOrderStatusFr(order.status)}
                </div>
                {order.invoice?.number ? <div className="text-sm text-stone-500">Facture {order.invoice.number}</div> : null}
              </div>
              <div className="flex items-center gap-4">
                <div className="text-sm font-semibold text-brand-900">{formatEuroCents(order.totalPriceCents)}</div>
                <Link className="underline text-sm" to={`/admin/orders/${order.id}`}>Voir la commande</Link>
              </div>
            </div>
          ))}
          {filteredOrders.length === 0 ? <p className="text-sm text-stone-500">Aucune commande dans ce filtre.</p> : null}
        </div>
      )}
    </section>
  );
