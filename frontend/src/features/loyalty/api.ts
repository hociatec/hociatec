...
import type { MyVoucherDto } from '@/features/vouchers/api';
...
export const fetchAdminLoyaltyCustomers = async (search = ''): Promise<AdminLoyaltyCustomerDto[]> => {
  ...
  if (isApiOk(data)) {
    return (data.data?.items ?? []) as AdminLoyaltyCustomerDto[];
  }
...
export const updateAdminLoyaltyCustomer = async (
  ...
  if (isApiOk(data)) {
    return data.data?.customer as AdminLoyaltyCustomerDto;
  }
