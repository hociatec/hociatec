...
final class TrivialClassesCoverageTest extends TestCase
{
    ...
    public function testSmallPersistenceHelpersDelegateToEntityManager(): void
    {
        ...
        $auditPersistence = new DoctrineUnitOfWork($entityManager);
        $orderPersistence = new DoctrineUnitOfWork($entityManager);

        $user = new User('ada@example.com', 'Ada', 'Lovelace', new \DateTimeImmutable('1990-01-01'), '0102030405', 'female');
        $auditPersistence->persist(new AuditRequest('AUD-1', $user, AuditType::TECHNICAL, 'https://example.com', 'Description'));
        $orderPersistence->persist(new OrderEvent(new Order('ORD-1', $user), 'created', 'Body', 9, 'Admin'));
    }
