...
namespace App\Module\Order\Command;
        ...
        foreach ($this->orders->findAll() as $order) {
            foreach ([['pdf', $order->getInvoicePdfPath()], ['xml', $order->getInvoiceXmlPath()]] as [$extension, $path]) {
                ...
                if (!preg_match('/^[A-Za-z0-9._-]+\\.'.preg_quote((string) $extension, '/').'$/i', $filename)) {
                    $output->writeln('<comment>Chemin ignoré pour la commande '.$order->getId().'.</comment>');
                    continue;
                }
                ...
                if (is_file($source) && !is_file($target) && !rename($source, $target)) {
                    $output->writeln('<error>Impossible de déplacer '.$filename.'.</error>');
