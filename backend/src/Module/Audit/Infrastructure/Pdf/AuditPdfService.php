...
class AuditPdfService implements AuditPdfRenderer
{
    public function renderDetailed(AuditRequest $audit): string
    {
        ...
        $html = $this->buildDetailedHtml($audit);
    ...
    public function renderSummary(AuditRequest $audit): string
    {
        ...
        $html = $this->buildSummaryHtml($audit);
        $dompdf = new \Dompdf\Dompdf();
        $dompdf->loadHtml($html, 'UTF-8');
        $dompdf->setPaper('A4', 'portrait');
        $dompdf->render();
    ...
    private function buildDetailedHtml(AuditRequest $audit): string
    {
        $grouped = [];
...
HTML;
    ...
    private function buildSummaryHtml(AuditRequest $audit): string
    {
        $statsByCat = [];
